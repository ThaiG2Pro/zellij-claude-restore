...
How enrichment works (all in `src/enrich.rs`, pure + deterministic; `main.rs` only wires the host I/O and the `auto_enter` flag). The plugin calls `enrich_layout(kdl, resolve, auto_enter) -> (String, EnrichStats)`; `enrich_claude_panes` is a back-compat wrapper (auto_enter=false):
