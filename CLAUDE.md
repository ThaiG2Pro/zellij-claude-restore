...
A Zellij plugin (Rust → WASM) that lets a developer **explicitly save a named workspace snapshot** capturing the Zellij layout **plus the Claude Code chat session ID running in each pane**, so that `zellij --layout <name>` after a reboot resumes the exact layout _and_ the exact Claude sessions.

The core problem: Zellij resurrection replays each pane's `command` from `/proc/<pid>/cmdline`, but `claude` launched bare (no resume flag in argv) spawns a _new_ chat. argv is immutable after `execve()`, so the running session ID can't be recovered from the process. The fix enriches the saved layout KDL with `args "--resume" "<uuid>"` per claude pane.

> **`--resume`, NOT `--session-id`.** Verified empirically (Jun 29): `claude --session-id <uuid>` is for _assigning_ an ID to a brand-new session — if that UUID already exists it errors `Session ID … is already in use` and refuses to start. `claude --resume <uuid>` is the flag that re-opens an existing session. The plugin injects `--resume`; `pane_has_session_id()` treats either flag as "already pinned" so we don't double-inject.
...
# artifact: target/wasm32-wasip1/release/zellij-claude-restore.wasm   (hyphen — it's a BINARY crate)
```
...
- The artifact basename follows the package name: `zellij-claude-restore.wasm` (hyphen). The old cdylib build produced `zellij_claude_restore.wasm` (underscore) — that name is stale, don't reference it.
...
- **Redeploying a rebuilt `.wasm` does NOT take effect in running sessions.** zellij caches the compiled plugin per session under `~/.cache/zellij/<session-uuid>/file:/<abs-wasm>/` (plus a global `~/.cache/zellij/file:/<abs-wasm>/`) and does **not** invalidate on file-content change, and a session that already loaded the plugin keeps the old instance in memory. After `cp …/zellij-claude-restore.wasm ~/.config/zellij/plugins/`, you MUST both (a) purge the cache: `find ~/.cache/zellij -type d -name 'zellij-claude-restore.wasm' -prune -exec rm -rf {} +`, and (b) test in a **freshly started** `zellij` session (not a reattach). The first `snap` in that new session recompiles the wasm (a few seconds) and may exceed `snap`'s `timeout 3` — just run `snap` again. Symptom of testing a stale plugin: code changes appear to have no effect. (`md5sum` the deployed file vs `target/wasm32-wasip1/release/…` to confirm the _file_ is current; the cache is the separate culprit.)
- The pure KDL-enrichment logic in src/enrich.rs has 47 unit tests (cargo test). The host/WASI flow (pipe/load/dump_session_layout/save_layout) has no automated coverage — verify it manually: cargo build --release --target wasm32-wasip1 → zellij pipe --plugin file:<abs-wasm> --name save -- <snapshot> → inspect ~/.config/zellij/layouts/<snapshot>.kdl → zellij --layout <snapshot>
...
```bash
zellij pipe --plugin file:~/.config/zellij/plugins/zellij-claude-restore.wasm --name save -- <snapshot-name>
```
...
How enrichment works (all in `src/enrich.rs`, pure + deterministic; `main.rs` only wires host I/O + config). The plugin calls `enrich_layout(kdl, resolve, &EnrichConfig) -> (String, EnrichStats)` where `EnrichConfig { auto_enter, claude_command }`; `enrich_claude_panes` is a back-compat wrapper (`EnrichConfig::default()`):
...
- **Match** `pane` nodes whose `command` **basename** equals `cfg.claude_command` (default `claude`, so `/usr/bin/claude` matches too). Configurable via `--args claude_command=…` / `$ZCS_CLAUDE_CMD` / plugin `load()` config, for renamed binaries (`claude-code`). Arg-wrappers (`npx claude`) are NOT matched (command basename is `npx`).
- **Skip template subtrees** — `new_tab_template`, `tab_template`, `swap_tiled_layout`, `swap_floating_layout`. Those describe what to spawn for a _brand-new_ tab; pinning them to an old session id would be wrong. Only the live `tab` panes get enriched.
...
- **Neutralize the snap pane.** `neutralize_snap_pane()` detects the pane that ran the `snap`/`zellij pipe … --name save` command itself (command basename `zellij` or `timeout`, args containing `save` + `pipe`/`zellij-claude-restore`) and strips its `command` + `args` so it restores as a plain shell. **Crucially it must also drop the `start_suspended` (and `close_on_exit`) CHILD nodes** — every command pane in a dump carries `start_suspended true` as a _child node_ (not a property), and zellij rejects `start_suspended` on a command-less pane with `start_suspended can only be set if a command was specified`, failing the whole save. Without neutralizing, restore re-runs `snap`, which hangs on the never-closed CLI pipe and re-overwrites the snapshot mid-restore.
- **Neutralize stray stdio panes (v0.4.3).** Zellij derives a dumped pane's `command` from its *current foreground process*, not what the user originally typed — so a pane whose real job is a plain interactive shell can get dumped mid-snapshot running a transient MCP/LSP subprocess an agent/editor spawned in it (e.g. `npm exec figma-developer-mcp --stdio`, `node …/typescript-language-server --stdio`). `neutralize_stdio_pane()` detects this by the literal `--stdio` arg (a flag specific to JSON-RPC-over-stdio processes; ordinary interactive commands don't take it) and strips it via the same shared `strip_command_pane()` helper the snap pane uses, since a `--stdio` process talks to one specific parent and can never be meaningfully resumed from a bare terminal replay.
- **Auto-enter (v0.2, default ON).** When `auto_enter` is true, `drop_start_suspended()` removes the `start_suspended` child **only from claude panes we pin** (enriched this run, or already carrying `--resume`/`--session-id`) — so `claude --resume …` launches on restore without ENTER. A claude pane we could NOT pin (no marker) keeps `start_suspended`, otherwise a bare `claude` would auto-start a *new* chat. Non-claude command panes always keep it — restore never auto-runs an unrecognized command. Disable per-snapshot via `--args auto_enter=false` (`snap --manual` / `$ZCS_NO_AUTO_ENTER`); then every command pane (incl. claude) keeps zellij's suspended default.
- **`EnrichStats`** ({`enriched`, `already_pinned`, `missing_marker`, `parse_failed`}) is returned by `enrich_layout` and written by `main.rs` as a one-line JSON status file at guest `/tmp/claude-sessions/.last-save.json` (host `/tmp/zellij-<uid>/…`); the `snap` helper reads it to report what happened instead of only checking the snapshot file exists.
...
- Guest `/tmp` maps to the host's `ZELLIJ_TMP_DIR` = **`/tmp/zellij-<uid>`**, _not_ real `/tmp`. So the `SessionStart` hook must write markers to `/tmp/zellij-<uid>/claude-sessions/<encoded-cwd>.session`.
...
Claude session storage for reference: `~/.claude/projects/<encoded-cwd>/<session-uuid>.jsonl`, append-only event log. Resume via `claude --resume <uuid>` (alias `claude -r <uuid>`). Do **not** use `claude --session-id <uuid>` to resume — it only assigns an ID to a _new_ session and errors on an existing UUID.
