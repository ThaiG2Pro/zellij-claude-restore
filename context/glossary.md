...
| Regression fixture | A KDL v1 test input encoding a historically-broken behavior (snap-pane neutralization, idempotent enrichment, template skip, cwd resolution) so it can never silently regress. | Lives in the `#[cfg(test)] mod tests` of `src/enrich.rs` (33 unit tests). |
